//
//  TutorialWorkoutDetailView.swift
//  HOLD
//
...
struct TutorialWorkoutDetailView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            trackOnboarding("ob_workout_detail", variant: UserStorage.onboarding)
            initializeExercise()
            startTimer()
    ...
    var workoutWithRests: Workout {
        ...
        for (index, exercise) in selectedWorkout.exercises.enumerated() {
            ...
            if index < selectedWorkout.exercises.count - 1 {
                let restExercise = Exercise.rest(seconds: 10)
